# Task T05: Agent — `planner`

## References
- Read: `../shared/interfaces.md` — §1 (work orders), §2 (independence CLI)
- Read: increment 1's `shared/interfaces.md` §3 — agent frontmatter shape
- Read: `../knowledge/iron-law.md` — §7, §8

## Dependencies
- Depends on: none (agent-frontmatter lint rule exists from increment 1; the independence
  CLI contract is fixed in `shared/interfaces.md` §2, so this file needs no code to exist)
- Depended on by: T09, T11

## The one rule that matters

**The locus is a promise, and the planner is the only one who makes it.** Everything
downstream — wave packing, worktree parallelism, per-commit locus checks, the meaning of a
`locus-breach` finding — rests on each work order's locus naming EVERY file the order may
create or modify. A planner that under-declares turns honest coders into breach escalations;
one that over-declares (blanket globs of the repo) serializes everything. Declare exactly
what the change needs, and route genuinely entangled work to `coupled` by touching the
shared-file list honestly.
...
- `model: opus` (the intelligence dial swaps to `fable` per call, never in frontmatter).
- Keep the "Run the partition, paste it raw" section unsoftened — `partition_raw` must be
  verbatim CLI output (interfaces §1).

## Negative Constraints (DO NOT)
- Do NOT give it Edit or Write — it decomposes; it never implements.
- Do NOT let it inherit: `model: opus` explicitly.

## Implementation Steps

- [ ] **Step 1: Write the agent**

Create `agents/planner.md` with exactly this content:
...
- [ ] **Step 3: Verify the fence** — `grep -n "^tools:" agents/planner.md` → exactly
  `tools: Read, Grep, Glob, Bash`; `grep -c "verbatim stdout" agents/planner.md` → `1`.
- [ ] **Step 4: Commit**

```bash
git add agents/planner.md
git commit -m "$(cat <<'EOF'
feat(agents): add planner

Decomposes into work orders with honestly declared loci, runs the
independence CLI itself, pastes the partition verbatim (IRON LAW §2).
...
## Acceptance Criteria
- [ ] `node tools/lint.mjs` exits 0; frontmatter matches exactly
- [ ] "verbatim stdout" and the AND-test title rule present unsoftened
- [ ] The verification-mode rule (Method §3) present, including "flagged for the gate,
      never converted into a synthetic test" and "the reviewer may enforce ONLY what your
      criteria name"
