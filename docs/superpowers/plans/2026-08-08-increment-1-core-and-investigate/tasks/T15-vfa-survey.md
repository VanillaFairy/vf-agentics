# Task T15: `vfa-survey` — the shared nested core

## References
- Read: `../shared/interfaces.md` — **§4 schemas, §5 coverage block, §6 the survey contract**
- Read: `../knowledge/iron-law.md` — §2, §3, §4, §5, §6, §8 all land in this file
- Read: `../shared/conventions.md` — "Workflow script constraints"
- Read: `../knowledge/run-lint.md`

## Dependencies
- Depends on: T04, T05, T06b, T07b, T08, T09 (all six workflow rules must be able to check this
  file) and T11, T12, T13, T14 (the agents it calls must exist)
- Depended on by: T16, and in later increments `vfa-diagnose`, `vfa-develop`, `vfa-ue-develop`

## Why this file is the centre of the design

Three of the four skills in this plugin — and the UE plugin's skill — call this workflow as their
evidence-gathering phase via `workflow('vfa-survey', …)`. It exists as a nested workflow rather
than copied text because the **resume loop** inside it is the subtlest code in the system and the
place IRON LAW §2 and §3 are actually enforced. Four hand-maintained copies would drift, and the
drift would be silent.

**Survey ends at evidence, never at an answer.** It returns findings and stops. Synthesis belongs
to its callers. If this ever returns prose conclusions, none of the four callers compose.

## Scope
**Files:**
- Create: `workflows/vfa-survey.workflow.js`

**BOUNDARY — you MUST NOT modify any files outside this list.**

## Positive Constraints (DO)
- Match `../shared/interfaces.md` §6 exactly — args in, return shape out.
- Every `agentType` fully qualified as `vf-agentics:<name>`.
- Every exit path returns the documented shape including `coverage`.
- `coverage.complete` is **derived in JS**, never taken from an agent.

## Negative Constraints (DO NOT)
- Do NOT `import` or `require` anything.
- Do NOT use `Date.now()`, argless `new Date()`, or `Math.random()` — they break run resumption.
- Do NOT write `minItems`/`maxItems`/`minLength`/`maxLength` in any schema.
- Do NOT cap any agent by turns or tool calls.
- Do NOT synthesize, conclude, or write prose answers. That is T16's job.
- Do NOT throw. On any failure, return the documented shape with the failure described inside it.

## Implementation Steps

- [ ] **Step 1: Confirm every rule that guards this file is in place**

Run: `node --test test/ && node tools/lint.mjs`
Expected: all tests pass, `OK: no findings`. If any rule task is incomplete, stop and escalate —
this file is written against those rules.

- [ ] **Step 2: Write the workflow**

Create `workflows/vfa-survey.workflow.js` with exactly this content:
...
function surveyResult(topics, verdicts, history, docs, coverage) {
  return { question, topics, verdicts, history, docs, coverage }
}
...
This is the first real exercise of six rules at once. If any fires, fix this file — **do not
weaken a rule.**

- [ ] **Step 4: Prove the rules are actually watching this file**

Each check below should produce exactly one finding. Revert after each; **commit none of them.**

| Temporary change | Expected rule |
|---|---|
| add `maxItems: 40` inside `HITS` | `no-schema-bounds` |
| add `maxTurns: 5` to a `agent()` options object | `no-turn-caps` |
| delete `coverage,` from `coverageOf`'s returned object | `coverage-block` |
| change `name: 'vfa-survey'` to `name: 'survey'` | `workflow-meta` |
| add `import { x } from 'y'` at the top | `no-imports` |
| change one `agentType` to a bare `'scout'` | `qualified-agent-types` |

Confirm `node tools/lint.mjs` reports `OK: no findings` after the last revert.

- [ ] **Step 5: Run the full suite**

Run: `node --test test/ && node tools/lint.mjs`
Expected: all tests pass, `OK: no findings`.

- [ ] **Step 6: Commit**

```bash
git add workflows/vfa-survey.workflow.js
git commit -m "$(cat <<'EOF'
feat(workflows): add vfa-survey, the shared evidence core

Nested workflow called by every skill in this plugin. Holds the single copy
of the resume-until-exhausted loop (IRON LAW §3) and derives coverage.complete
in JS rather than trusting an agent's self-report (§2, §4). Ends at evidence,
never at an answer, so all four callers compose.
...
- [ ] All six negative checks in Step 4 fired the expected rule, and all were reverted
- [ ] Every exit path returns the §6 shape including `coverage`
- [ ] `coverage.complete` is computed in `coverageOf`, never read from an agent response
- [ ] Every `agentType` is `vf-agentics:*`
- [ ] No imports, no `Date.now()`, no `Math.random()`, no schema length bounds, no turn caps
- [ ] The workflow returns no prose conclusion
