...
//
// Design: docs/DESIGN.md#verification.
//
...
// is stated here, and the notes say how many changed files matched, so
 ...
 * question for a test written to hold something already correct in place — an order marked
 * `pins: 'data'`. That marking removes the false alarm and, alone, leaves the right question
 * unasked by anything mechanical: the planner writes the mutation into the acceptance criteria
 * as prose and the coder performs it by hand. This is that question, executed.
 *
...
export function verify(options) {
  ...
  // (the field list does not move), and '' is honest: not "the suite
...
// this script applied to its own record: this line carries ten fields including three
...
// all between them — and the line's shape is unchanged.
//
...
function journalLine(options, payload, head) {
  return {
    ...
    build: payload.build,
    // This field has to be on the line: `verifiable` reads `typecheck`
    ...
    // Same reason `typecheck` is here: `plainVerifyOk` reads it, and a
