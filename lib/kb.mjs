...
if (isMain) {
  try {
    ...
    if (!command || !repo) {
      throw new Error('usage: kb.mjs chain|verify|compact <repo> [paths...] | kb.mjs append <repo> --digest <hex> --b64 <token>')
    }
    ...
    if (command === 'append') {
      const raw = decodeB64(flag(process.argv, 'b64'))
      let parsed = null
      try {
    ...
    if (!payload) {
      throw new Error('unknown command ' + JSON.stringify(command) +
        ' — expected chain, verify, compact or append')
    }
