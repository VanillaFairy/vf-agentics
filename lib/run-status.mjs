...
export function deriveRun(runstamp, plan, state = [], landedInTree = null, readNotes = []) {
  ...
  const verifiedLines = state.filter((e) => e.kind === 'order-verified')
  ...
  if (!plan) {
    return {
      ...
      approved_unmerged: [], verified_unapproved: [],
  ...
  // Every line counts as state, whatever its kind. A run that died mid-wave has verified or
  // approved orders and no wave line — calling it `planned` invites re-planning work that is
  // sitting green and reviewed on its branches, which is exactly the wrong invitation for the
  // one run most worth resuming.
  const status = note
    ? 'unreadable'
  ...
  return {
    ...
    verified_unapproved: [...new Set(verifiedLines.map((e) => e.order).filter(Boolean))]
