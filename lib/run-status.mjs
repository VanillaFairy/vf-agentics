...
export function deriveRun(runstamp, plan, state = [], landedInTree = null, readNotes = []) {
  ...
  // Two line types share the log. A line with no `kind` predates the second type, and every
  ...
  if (!plan) {
    return {
      ...
      approved_unmerged: [], waves_total: 0, waves_recorded: waveLines.length,
  ...
  // order merges exactly once. Escalated is a union for the same reason.
  const merged = [...new Set(waveLines.flatMap((e) => e.merged || []))]
  ...
  // Order lines count as state. A run that died mid-wave has approved orders and no wave
  // line — calling it `planned` invites re-planning work that is sitting reviewed on its
  // branches, which is exactly the wrong invitation for the one run most worth resuming.
  const status = note
    ? 'unreadable'
    : statusOf({ hasState: waveLines.length > 0 || orderLines.length > 0, unreached, landedInTree })
  ...
  return {
    ...
    waves_total: waves,
    waves_recorded: waveLines.length,
...
export function readRuns(root) {
  ...
  return dirs.map((runstamp) => {
    ...
    // no integration head, and reading its empty string would ask git about nothing.
    const waveLines = state.filter((e) => (e.kind || 'wave') === 'wave')
