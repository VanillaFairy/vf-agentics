...
//
// Contract: docs/superpowers/specs/2026-08-27-increment-9-contracts.md §3.
//
...
/** The order's lifecycle, in order. `next_action` always names one of these, or 'none'. */
export const ACTIONS = ['code', 'verify', 'review', 'merge']
...
export function replay(state, journal, orderById) {
  ...
  // Increment 6 §1's rule reaching across the two files, which it can only do because both
...
export function measuredOf(entry) {
  const out = []
  if (entry.build === 'passed' || entry.build === 'absent') out.push('build')
  if (entry.suite === 'passed' || entry.suite === 'absent') out.push('suite')
...
export function nextActionFor(id, wo, r, git) {
  ...
  const note = (next, why) => ({ next_action: next, stage_note: why })

  if (r.landed.has(id)) return note('none', 'merged by an earlier invocation, per the run log')
  ...
  if (branchFacts && branchFacts.already_merged) {
    ...
    if (witnessed) return note('none', 'already in the integration branch; the merge was made but never recorded')
  }
...
export function deriveVerdict(runstamp, plan, stateRaw, journalRaw, git, readNotes = []) {
  ...
  const orders = workOrders.filter((wo) => wo && wo.id).map((wo) => {
    ...
    const { next_action, stage_note } = nextActionFor(wo.id, wo, r, git)
 ...
 */
export function gitFacts(root, runstamp, orderIds, hasRecords) {
  ...
  for (const id of started) {
    ...
    const forkPoint = integrationHead ? git(root, ['merge-base', branch, integrationBranch]) : { ok: false, out: '' }
    ...
    const log = base
      ? git(root, ['log', '--reverse', '--format=%H%x09%s', base + '..' + branch])
      : { ok: false, out: '' }
...
export function verdictFor(root, runDir) {
  const notes = []
  ...
  if (!plan) return deriveVerdict(runstamp, null, '', '', { clean: false, branches: {} }, notes)
  ...
  return { payload, payload_digest: fnv1a(canonical(payload)) }
}
