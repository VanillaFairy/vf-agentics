...
if (isMain) {
  try {
    ...
    if (command === 'append') {
      // --b64 when the caller could encode the line (the workflow mints state lines whole);
      // stdin when it could not (a journal line is filled in by the agent that observed it,
      // so there is nothing to encode ahead of time). Both end in the same two checks.
      const token = flag(process.argv, 'b64')
      const out = appendLine(runDir, flag(process.argv, 'file') || 'state',
        token ? decodeB64(token) : readStdin(), flag(process.argv, 'digest'))
